...
- The runner composes trait selectors from `site`+`env`; the user never writes `Site=`/`Env=`/`Kind=` by hand — only the `actions` slice (`Category`/`Module` keys only, `&`/`|` combinators).
